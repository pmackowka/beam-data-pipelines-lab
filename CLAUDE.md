...
A personal Apache Beam (Python SDK) learning log: numbered Jupyter notebooks (`001`–`025`) plus two script-only folders, covering batch/streaming transforms, windowing, Pub/Sub streaming, and deployment to Google Cloud Dataflow/BigQuery. See `README.md` (Polish) for the full topic-by-topic table of contents.
...
- **File naming**: `NNN_Descriptive_English_Title.ipynb` (or `.py`), underscores only, Title_Case, no spaces/parentheses. Numbers are chronological, not thematic — don't renumber existing files when adding a new one, just continue from the highest existing number.
